...
class Geomformer(nn.Module):
    ...
    **kwargs,
    ...
    def forward(self, batched_data):
        ...
        # pbc_dict1 = self.construct_pbc_expand_batched(batched_data, True, self.use_per_atom_outcell_index)
        # pbc_dict2 = self.cell_expander.expand(
        #         pos,
        #         pbc,
        #         atomic_numbers[:, :, 0],
        #         cell,
        ...
        #         use_local_attention=True,
        #     )
        # # 比较pbc_dict1和pbc_dict2的内容是否相等，哪些内容不相等
        # for key in pbc_dict1.keys():
        #     if key not in pbc_dict2:
        #         print(f"Key {key} is missing in pbc_dict2")
        #     elif not torch.equal(pbc_dict1[key], pbc_dict2[key]):
        #         print(f"Key {key} is different between pbc_dict1 and pbc_dict2")
